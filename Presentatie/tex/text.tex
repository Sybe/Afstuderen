...
\begin{frame}{Modelchecking}
\begin{itemize}
	\item Models a system, program, protocol, etc...
	\item Check if model meets specifications
	\item Problem: State space explosion
	\begin{itemize}
		\item Grows exponentially with size of model
	\end{itemize}
	\item Timed Automata adds time to these models	
\end{itemize}
\end{frame}
...
\begin{subfigure}[b]{.4\textwidth}
	...
	\begin{subfigure}[b]{.4\textwidth}
		\begin{itemize}
			\item Diagram to represent integer variables
...
\begin{figure}[h]
	\begin{tikzpicture}[
		...
		rotate=90, xshift =-0.4 cm, yshift = -0.2cm]{$\leq 4$};
		\draw[->] (S2) --(S5) node [midway, above, sloped, scale=0.75,
		rotate=90, xshift =-0.4 cm, yshift = -0.2cm]{$\leq 5$};
		\draw[dashed,->] (S0) --(S1) node [midway, above, sloped, scale=0.75,
		rotate=0, xshift =-0.7 cm, yshift = -0.2cm]{};
...
\begin{figure}
	...
	\begin{tikzpicture}[
		...
		rotate=0, xshift =-0.4 cm, yshift = -0.2cm]{};
		\draw[->] (S1) --(S2) node [midway, above, sloped, scale=0.75,
		rotate=90, xshift =-0.6 cm, yshift = -0.2cm]{$<-1$};
		\draw[->] (S2) --(S3) node [midway, above, sloped, scale=0.75,
		rotate=90, xshift =-0.6 cm, yshift = -0.2cm]{$<-1$};
		\draw[->] (S3) --(S4) node [midway, above, sloped, scale=0.75,
		rotate=90, xshift =-0.4 cm, yshift = -0.2cm]{$<3$};
